...
> **Status: Active** — This protocol is the minimum independent review gate for durable L1/L2 work. Use Superpowers `requesting-code-review` skill to fulfill the reviewer role when no human reviewer is available.
...
**Enforcement for L1/L2 durable tasks**: When a durable feature task (L1 or L2) completes with real implementation code, invoke `Superpowers requesting-code-review` skill before marking the task done. Self-review is only acceptable when independent review is genuinely unavailable and the task scope is purely docs/config — document this in the review.md record.
